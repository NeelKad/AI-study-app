...
def create_reel():
    try:
        ...
        if not segments:
            segments = [note_text[:200]]  # Fallback

        # Background colors based on style
        if style == "aesthetic":
            bg_colors = [(30, 27, 75), (49, 46, 129), (67, 56, 202)]
        elif style == "bold":
            bg_colors = [(17, 24, 39), (31, 41, 55), (55, 65, 81)]
        else:  # study-vibes
            bg_colors = [(15, 23, 42), (51, 65, 85), (71, 85, 105)]
        ...
        # Export video (without audio for now to avoid music file issues)
        ...
        return send_file(
            ...
            mimetype="video/mp4", 
            as_attachment=True, 
            download_name="study_reel.mp4"
        )

    except Exception as e:
        print(f"Error creating reel: {str(e)}")
        return jsonify({"error": f"Failed to create reel: {str(e)}"}), 500

# Fixed route for create_reel page
@app.route('/create_reel')
@app.route('/create_reel/<int:note_id>')
@login_required
@trial_required
def create_reel_page(note_id=None):
    note_content = ""
    if note_id:
        note = Note.query.filter_by(id=note_id, user_id=current_user.id).first()
        if note:
